...
; 标准FAT12格式软盘专用的代码 Stand FAT12 format floppy code
		...
		DW		1				; boot记录所占大小，FAT起始位置（一般为第一个扇区）
		DB		2				; FAT个数（必须为2）FAT分为FAT1、FAT2，每个FAT为9个扇区的占位。
		DW		224				; 根目录文件最大数（一般为224项）
		...
		DD		0				; 不使用分区，必须是0
		DD		2880			; 重写一次磁盘大小
		DB		0,0,0x29		;（INT 13H的驱动号、保留未使用、扩展引导标记）
		DD		0xffffffff		; 卷标号码
		DB		"BANANAOS   "	; 磁盘的名称（必须为11字节，不足填空格）
		DB		"FAT12   "		; 磁盘格式名称（必须为8字节，不足填空格）
								; 引导代码以及其他数据大小为448字节（引导代码之后剩余空间用0补齐）
		RESB	18				; 先空出18字节
